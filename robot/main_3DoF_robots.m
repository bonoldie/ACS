...
test_q_values = [-pi/3 -0.11 pi/8]';
evaluatedKinematics = evaluateKinematics(kinematics, test_q_values);

Jg_b_1 = geometricJacobianTo(evaluatedKinematics, '1')
geometricJacobian(robot, test_q_values, 'Link2')
...
test_q_dot_values = [1, 2, 3]';

links_mass = [1 1 1 1]';

B = inertiaMatrix(kinematics);

% kineticEnergy = 0.5*test_q_dot_values'*B*test_q_dot_values;